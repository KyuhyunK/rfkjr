...
// C/C++ source code generated on : Thu Nov 07 10:33:31 2024
//
  ...
  namespace matlab
  {
    ...
    void NodeInterface::initialize(int argc, char ** argv)
    {
      try {
        ...
        ROS_INFO("** Starting the model \"rfkjr\" **\n");

        // initialize the model which will initialize the publishers and subscribers
        rtmSetErrorStatus(rfkjr_M, (NULL));
        rfkjr_initialize();
    ...
    boolean_T NodeInterface::getStopRequestedFlag(void)
    {
      ...
      return (!(rtmGetErrorStatus(rfkjr_M)
                == (NULL)));

#else

      return (!(rtmGetErrorStatus(rfkjr_M)
                == (NULL)) || rtmGetStopRequested(rfkjr_M));
    ...
    {
      mRunModel = (rtmGetErrorStatus(rfkjr_M) ==
                   (NULL));
      while (mRunModel) {
        mBaseRateSem.wait();
